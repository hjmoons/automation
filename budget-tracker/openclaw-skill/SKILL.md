...
사용자가 "식비 예산 50만원으로 해줘", "이번 달 교통비 예산 15만원 잡아줘" 같이 카테고리별 예산 설정을 요청하면:

1. `GET /categories/`로 이름이 맞는 지출 카테고리의 `id`를 찾는다. 해당하는 카테고리가 없으면 먼저 카테고리부터 만들지 물어본다.
2. 몇 년/몇 월 예산인지 확인한다. 명시하지 않으면 이번 달로 간주한다.
3. 이미 그 카테고리+연+월에 예산이 등록돼 있는지 확인한다:
   ```bash
   curl -s "http://localhost:8000/budgets/?year=<연도>&month=<월>"
   ```
   결과에 같은 `category_id`가 이미 있으면 새로 만들지 말고 아래 "예산 수정" 절차로 넘어간다.
4. 없으면 새로 등록한다:
   ```bash
   curl -s -X POST http://localhost:8000/budgets/ \
     -H "Content-Type: application/json" \
     -d '{"category_id": <카테고리ID>, "year": <연도>, "month": <월>, "amount": <금액>}'
   ```
5. 등록 완료 후 결과를 사용자에게 알려준다.

## 예산 수정

사용자가 "식비 예산 좀 늘려줘", "이번 달 카페 예산 8만원으로 바꿔줘" 같이 요청하면:

1. "예산 등록" 3번 절차(`GET /budgets/?year=&month=`)로 대상 예산의 `id`를 찾는다. 없으면 새로 등록할지 물어본다.
2. 수정한다:
   ```bash
   curl -s -X PATCH http://localhost:8000/budgets/<예산ID> \
     -H "Content-Type: application/json" \
     -d '{"amount": <새 금액>}'
   ```
3. 수정 결과를 사용자에게 알려준다.
...
사용자가 "식비 예산 없애줘" 같이 요청하면:

1. "예산 등록" 3번 절차로 대상 예산의 `id`를 찾는다. 일치하는 게 없으면 그렇게 알린다.
2. 삭제는 되돌릴 수 없으므로, 실행 전에 반드시 "OO 예산을 삭제할까요?"라고 되물어 사용자 확인을 받는다.
3. 확인되면 삭제한다:
   ```bash
   curl -s -o /dev/null -w "%{http_code}" -X DELETE http://localhost:8000/budgets/<예산ID>
   ```
   - `204`면 성공, `404`면 해당 항목이 없다는 뜻.
4. 결과를 사용자에게 알려준다.
